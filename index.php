...
if (isset($_GET['logout'])) {
    ...
    fm_redirect("http://prepamistral.fr");
...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        <div class="path login-form">
                <img src="logo_mistral.png" alt="Explorateur de fichiers" height="150" width="150" style="margin:20px;">
            <form action="" method="post">
                <label for="fm_usr">Nom d'tilisateur</label><input type="text" id="fm_usr" name="fm_usr" value="" placeholder="Nom d'utilisateur" required><br>
                <label for="fm_pwd">Mot de passe</label><input type="password" id="fm_pwd" name="fm_pwd" value="" placeholder="Mot de passe" required><br>
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Le dossier <b>%s</b> est supprimé' : 'Le fichier <b>%s</b> est supprimé';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? "Le dossier <b>%s</b> n'est pas supprimé" : "Le fichier <b>%s</b> n'est pas supprimé";
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if($_GET['type']=="file") {
            ...
            if (fm_mkdir($path . '/' . $new, false) === true) {
                fm_set_msg(sprintf('Dossier <b>%s</b> créé', $new));
            } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                fm_set_msg(sprintf('Le dossier <b>%s</b> existe déjà', fm_enc($new)), 'alert');
            } else {
                fm_set_msg(sprintf("Le dossier <b>%s</b> n'a pas pu être créé", fm_enc($new)), 'error');
            }
        }
    } else {
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('Renommé <b>%s</b> en <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('Erreur pour renommer <b>%s</b> en <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nom non rempli', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <div class="path">
        <p><b>Fichiers téléversés</b></p>
        <p class="break-word">Dossier de sestination: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?></p>
...
if (isset($_POST['copy']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            ?>
            <p class="break-word">Fichiers: <b><?php echo implode('</b>, <b>', $copy_files) ?></b></p>
            <p class="break-word">Dossier source: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
                <label for="inp_copy_to">Dossier de destination:</label>
                <?php echo FM_ROOT_PATH ?>/<input type="text" name="copy_to" id="inp_copy_to" value="<?php echo fm_enc(FM_PATH) ?>">
            </p>
            <p><label><input type="checkbox" name="move" value="1"> Déplacer'</label></p>
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> Copier </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> Annuler</a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        <p><b>Copying</b></p>
        <p class="break-word">
            Chemin d'accès: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Dossier de destination: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            Chemin complet: <?php echo fm_enc(fm_convert_win($file_path)) ?><br>
            Taille: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
           MIME-type: <?php echo $mime_type ?><br>
            ...
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                Fichiers dans l'archive: <?php echo $total_files ?><br>
                Taille totale: <?php echo fm_get_filesize($total_uncomp) ?><br>
                Taille dans l'archive: <?php echo fm_get_filesize($total_comp) ?><br>
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="fa fa-cloud-download"></i> Télécharger</a></b> &nbsp;
            <b><a href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> Ouvrir</a></b> &nbsp;
            <?php
            // ZIP actions
            if (!FM_READONLY && $is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> Extraire</a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="Extraire vers <?php echo fm_enc($zip_name) ?>"><i class="fa fa-check-circle"></i>
                    Extraire vers un dossier</a></b> &nbsp;
                <?php
            }
            if($is_text && !FM_READONLY) {
            ?>
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> Edition</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace" class="edit-file"><i class="fa fa-pencil-square"></i> Edition avancée</a></b> &nbsp;
            <?php }
            ...
            <?php } ?>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-chevron-circle-left"></i> Retour</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
...
if (isset($_GET['edit'])) {
    ...
    <div class="path">
        <div class="edit-file-actions">
            <a title="Annuler" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> Annuler</a>
            <a title="Sauvegarder" href="javascript:backup('<?php echo urlencode($path) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-database"></i> Sauvegarder</a>
            <?php if($is_text) { ?>
                <?php if($isNormalEditor) { ?>
                    <a title="Avancé" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-paper-plane"></i> Editeur avancé</a>
                    <button type="button" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Enregistrer</button>
                <?php } else { ?>
                    <a title="Editeur simple" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> Editeur simple</a>
                    <button type="button" name="Enregistrer" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> Enregistrer</button>
                <?php } ?>
            <?php } ?>
        </div>
        <?php
...
<th>Nom</th><th style="width:10%">Taille</th>
<th style="width:12%">Modifié le</th>
<?php if (!FM_IS_WIN): ?><th style="width:6%">Perms</th><th style="width:10%">Propriétaire</th><?php endif; ?>
...
<td>Dossier</td><td><?php echo $modif ?></td>
<?php if (!FM_IS_WIN): ?>
...
<a title="Supprimer" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Supprimer le dossier?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
<a title="Renommer" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
<a title="Copier vers..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
<?php endif; ?>
<a title="Lien direct" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
</td></tr>
    <?php
    flush();
}
...
<tr>
<?php if (!FM_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td><?php endif; ?>
<td><div class="filename"><a href="<?php echo $filelink ?>" title="Informations sur le fichier"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div></td>
...
<?php if (!FM_READONLY): ?>
<a title="Supprimer" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Supprimer le fichier?');"><i class="fa fa-trash-o"></i></a>
<a title="Renommer" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
<a title="Copier vers..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
<?php endif; ?>
<a title="Lien direct" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
<a title="Télécharger" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
if (empty($folders) && empty($files)) {
    ?>
<tr><?php if (!FM_READONLY): ?><td></td><?php endif; ?><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em><?php echo 'Le dossier est vide' ?></em></td></tr>
<?php
} else {
    ?>
<tr><?php if (!FM_READONLY): ?><td class="gray"></td><?php endif; ?><td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
Taille: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
fichiers: <?php echo $num_files ?>,
dossiers: <?php echo $num_folders ?>
</td></tr>
<?php
}
?>
...
<?php if (!FM_READONLY): ?>
<p class="path footer-links"><a href="#/select-all" class="group-btn" onclick="select_all();return false;"><i class="fa fa-check-square"></i> Sélectionner tout</a> &nbsp;
<a href="#/unselect-all" class="group-btn" onclick="unselect_all();return false;"><i class="fa fa-window-close"></i> Déselectionner tout</a> &nbsp;
<a href="#/invert-all" class="group-btn" onclick="invert_all();return false;"><i class="fa fa-th-list"></i> Inverser la sélection</a> &nbsp;
<input type="submit" class="hidden" name="delete" id="a-delete" value="Delete" onclick="return confirm('Supprimer les fichiers et dossiers sélectionnés')">
<a href="javascript:document.getElementById('a-delete').click();" class="group-btn"><i class="fa fa-trash"></i> Supprimer </a> &nbsp;
...
<a href="javascript:document.getElementById('a-copy').click();" class="group-btn"><i class="fa fa-files-o"></i> Copier </a>
<a href="https://github.com/prasathmani/tinyfilemanager" target="_blank" class="float-right" style="color:silver">Explorateur de fichiers</a></p>
<?php endif; ?>
</form>
...
<div class="path main-nav">
        ...
        <?php if (!FM_READONLY): ?>
          <a title="Prépa Mistral" href="http://prepamistral.fr"><i class="fa fa-home"></i></a>
        <a title="Rechercher" href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-search"></i></a>
        <a title="Charger des fichiers" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i></a>
        <a title="Nouveau dossier" href="#createNewItem" ><i class="fa fa-plus-square"></i></a>
        <?php endif; ?>
        <?php if (FM_USE_AUTH): ?><a title="Se déconnecter" href="?logout=1"><i class="fa fa-sign-out" aria-hidden="true"></i></a><?php endif; ?>
        </div>
</div>
<?php
}
...
<div id="wrapper">
  <div id="createNewItem" class="modalDialog"><div class="model-wrapper"><a href="#close" title="Close" class="close">X</a><h2>Create New Item</h2><p>
        <label for="newfile">Item Type &nbsp; : </label><input type="radio" name="newfile" id="newfile" value="file">File <input type="radio" name="newfile" value="folder" checked> Dossier<br><label for="newfilename">Item Name : </label><input type="text" name="newfilename" id="newfilename" value=""><br>
